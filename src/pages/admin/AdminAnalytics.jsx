...
export default function AdminAnalytics() {
  ...
  useEffect(() => {
    ...
    setReports(getStorage("apex_reports", []));
  }, []);
  ...
  const lineOptions = {
    ...
    xAxis: { categories: [...new Set(reports.map((r) => r.date))].sort(), labels: { style: { color: "var(--text-base)", fontSize: "10px" } }, lineColor: "var(--border-base)" },
    yAxis: { title: { text: null }, gridLineColor: "var(--border-base)", labels: { style: { color: "var(--text-base)" } }, allowDecimals: false },
    tooltip: { backgroundColor: "var(--card-base)", style: { color: "var(--text-base)" }, borderColor: "var(--border-base)" },
    series: [{
      name: "Reports",
      data: [...new Set(reports.map((r) => r.date))].sort().map((d) => reports.filter((r) => r.date === d).length),
      color: "var(--secondary)",
      marker: { fillColor: "var(--secondary)", lineColor: "var(--secondary)" }
    }],
    credits: { enabled: false },
    legend: { itemStyle: { color: "var(--text-base)" } }
  };
    ...
    <div className="space-y-5 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-[var(--card-base)] border border-[var(--border-base)] rounded-2xl p-5">
          <HC highcharts={Highcharts} options={pieOptions} />
        </div>
        <div className="bg-[var(--card-base)] border border-[var(--border-base)] rounded-2xl p-5">
          <HC highcharts={Highcharts} options={barOptions} />
        </div>
        <div className="bg-[var(--card-base)] border border-[var(--border-base)] rounded-2xl p-5">
          <HC highcharts={Highcharts} options={lineOptions} />
        </div>
        <div className="bg-[var(--card-base)] border border-[var(--border-base)] rounded-2xl p-5">
          <HC highcharts={Highcharts} options={ratingOptions} />
        </div>
      </div>
    </div>
  );
