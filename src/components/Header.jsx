...
export default function Header() {
  ...
  useEffect(() => {
    const all = getStorage("apex_notifications", []);
    setNotifications(all.filter((n) => !n.employeeId || n.employeeId === currentUser?.id));
  }, [currentUser]);
  ...
  const markAll = () => {
    const all = getStorage("apex_notifications", []);
    const updated = all.map((n) => (!n.employeeId || n.employeeId === currentUser?.id ? { ...n, read: true } : n));
    setStorage("apex_notifications", updated);
    setNotifications(updated.filter((n) => !n.employeeId || n.employeeId === currentUser?.id));
  };
    ...
    <header className="h-16 sticky top-0 z-30 bg-[var(--card-base)]/90 backdrop-blur-xl border-b border-[var(--border-base)] px-5 flex items-center justify-between shrink-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button
            onClick={() => { setShowTheme(!showTheme); setShowFont(false); setShowNotif(false); }}
          ...
          >
            <RiPaletteLine className="text-base" />
        ...
        <div className="relative">
          <button
            onClick={() => { setShowFont(!showFont); setShowTheme(false); setShowNotif(false); }}
          ...
          >
            <RiFontSize className="text-base" />
        ...
        <div className="relative">
          <button
            onClick={() => { setShowNotif(!showNotif); setShowTheme(false); setShowFont(false); }}
              ...
              <div className="absolute right-0 mt-2 w-80 z-50 bg-[var(--card-base)] border border-[var(--border-base)] rounded-2xl shadow-2xl p-3 animate-slideUp">
                ...
                <div className="max-h-64 overflow-y-auto space-y-2">
                  {notifications.length === 0 ? (
                    ...
                    <div key={n.id} className={`p-3 rounded-xl border text-xs transition-all relative ${n.read ? "border-[var(--border-base)]/40 opacity-50" : "border-[var(--primary)]/20 bg-[var(--primary)]/5"}`}>
                      ...
                      <p className="text-[10px] opacity-40 mt-1.5 font-bold">{new Date(n.date).toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              </div>
