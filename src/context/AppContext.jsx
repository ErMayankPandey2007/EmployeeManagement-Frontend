...
import { initStorage, getStorage, setStorage } from "../utils/mockData";
...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    initStorage();
    const u = getStorage("apex_current_user", null);
  ...
  const login = (user) => {
    setCurrentUser(user);
    setStorage("apex_current_user", user);
  };
  ...
  return (
    <AppContext.Provider value={{ currentUser, login, logout, selectedTheme, setSelectedTheme, selectedFont, setSelectedFont, sidebarOpen, setSidebarOpen }}>
      {children}
    </AppContext.Provider>
  );
}
